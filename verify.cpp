...
int main(int argc, char **argv) {
  ...
  resultin.read(reinterpret_cast<char *>(u),
                N * N * N * sizeof(complex<float>));
  resultin.close();
  ...
  for (int i = 0; i < N * N * N; i++) {
    avg += u[i];
    avgref += r[i];
    if (isnan(real(u[i])) || isnan(imag(u[i])) ||
        (norm(r[i] - u[i]) / norm(r[i]) > 1.0e-3)) {
      errors++;
      if (errors <= 10) {
        printf("Error #%03d @ %d = %g\n", errors, i,
               norm(r[i] - u[i]) / norm(r[i]));
        printf("ref[%d] = (%g,%g),  computed[%d] = (%g,%g)\n", i, real(r[i]),
               imag(r[i]), i, real(u[i]), imag(u[i]));
      }
      pass = 0;
    }
  }
