...
chrono::high_resolution_clock::time_point fwd_t0, fwd_t1, adj_t0, adj_t1,
    wall_t0, wall_t1;
...
int main(int argc, char **argv) {
  ...
  for (int s = 0; s < S; s++) {
    for (int k = 0; k < K; k++) {
      raw[s * K + k] =
          complex<float>(temp[2 * (s * K + k)], temp[2 * (s * K + k) + 1]);
    }
  }
  delete[] temp;
  temp = NULL;
  ...
  int offsetZ = -35;
  NUFFT3D *nufft =
      new NUFFT3D(N, OF, wx, wy, wz, P, prechopX, prechopY, prechopZ, postchopX,
                  postchopY, postchopZ, offsetX, offsetY, offsetZ, W, L);
  ...
  adj_t1 = Clock::now();
  cout << "ADJ time = \t\t"
       << chrono::duration_cast<chrono::microseconds>(adj_t1 - adj_t0).count() /
              1e6
       << "\t\t sec" << endl;
  cout << "=================================================" << endl;
  ...
  delta_old = 0;
  for (int i = 0; i < N * N * N; i++) delta_old += norm(r[i]);
  for (int iter = 0; iter < maxCGIter; iter++) {
    ...
    fwd_t1 = Clock::now();
    cout << "FWD time = \t\t"
         << chrono::duration_cast<chrono::microseconds>(fwd_t1 - fwd_t0)
                    .count() /
                1e6
         << "\t\t sec" << endl;
    cout << "-------------------------------------------------" << endl;
    ...
    adj_t1 = Clock::now();
    cout << "ADJ time = \t\t"
         << chrono::duration_cast<chrono::microseconds>(adj_t1 - adj_t0)
                    .count() /
                1e6
         << "\t\t sec" << endl;
    for (int i = 0; i < N * N * N; i++) Ap[i] += lambda * p[i];
    den = epsilon;
    for (int i = 0; i < N * N * N; i++) den += conj(p[i]) * Ap[i];
    alpha = delta_old / den;
    for (int i = 0; i < N * N * N; i++) {
      u[i] = u[i] + alpha * p[i];
      r[i] = r[i] - alpha * Ap[i];
    ...
    delta = 0;
    for (int i = 0; i < N * N * N; i++) delta += norm(r[i]);
  ...
  cout << "Total Computation Time:" << endl
       << "\t\t"
       << chrono::duration_cast<chrono::microseconds>(wall_t1 - wall_t0)
                  .count() /
              1e6
       << "\t sec" << endl;
  delete nufft;
